...
const SectionComponent1 = () => {
  ...
  useEffect(() => {
    console.info('useEffect configuration page 4 ', configuration)
    if(survey) {
      const cSurvey = survey.content.components.find(x=> x.id === 'c-survey');
      const section = cSurvey.object.sections.find(x=> x.name === 'Your current HIV treatment');
